...
**사고 배경**: 2026-05-25 Post #7 FSM 통합 라이브 사고 — 게임 코드 변경 후 로컬 플레이 검증 없이 PATCH 진행 → 광 카드 잔존 버그 미검출 → 라이브 오염.

| 규칙 | 내용 |
|---|---|
| **자가 검증 의무** | 게임 코드 작성·수정 후 반드시 로컬 wrapper.html에서 직접 플레이 검증 |
| **PASS 기준** | 페이지 진입 → 난이도 모달 정상 출현 → 난이도 선택 → 게임 진행 → 광 카드·전체 카드 정상 동작 (FAIL 패턴 0건) |
| **납품 조건** | 로컬 PASS 결과(PASS/FAIL + 검증 항목) 첨부 없이 lead에 납품 금지 |
| **FAIL 시** | 라이브 PATCH 시도 금지 — 로컬에서 재작업 후 PASS 획득 후 납품 |

**납품 시 첨부 양식**:
```
[로컬 검증 결과]
- 환경: 로컬 wrapper.html / 브라우저 [버전]
- 페이지 진입: PASS
- 난이도 모달 출현: PASS
- 난이도 선택 → 게임 전환: PASS
- 카드 전체 동작 (광 포함): PASS
- FAIL 패턴: 0건
- 결론: PASS → 라이브 PATCH 가능
```

> ⚠️ 로컬 검증 없이 납품한 코드 = 미완성 산출물. lead가 PATCH 위임 브리프에서 로컬 PASS 미확인 시 PATCH 불가.
...
| 규칙 | 내용 |
|---|---|
| **정본 파일명** | `widgets/post{N}_widget.html` 1개만 존재 |
| **suffix 금지** | `_v1`, `_v5b`, `_backup`, `_old` 등 버전 suffix 파일명 금지 |
| **버전 히스토리** | git 커밋으로 관리. 파일명으로 버전 구분 금지 |
| **minify 버전** | `_wp.html` 구분 파일도 금지 — minify 결과를 같은 파일에 커밋 |
| **작업 전 확인** | `git log --oneline widgets/post{N}_widget.html`으로 최신 SHA 확인 후 착수 |

**사고 재발 방지**: 2026-05-25 v5b 베이스 선택 오류는 파일명 난립이 직접 원인. 정본이 하나뿐인 구조로 구조적 차단.

### Q2. LFS 도입 임계값 — 50포스트/200MB 초과 시

game 위젯은 HTML/JS 텍스트이므로 LFS 대상 아님. design 에이전트 PNG 자산이 50포스트 초과 또는 저장소 200MB 초과 시 dev 에이전트에 LFS 도입 요청. game 작업 절차 변경 없음.
...
| hook 항목 | 검증 명령 |
|---|---|
| G1. BH_ prefix 검증 | `grep -rn 'id="\|class="' widgets/post*_widget.html \| grep -v 'BH_\|""\|wp-' && echo "FAIL: BH_ prefix 누락" && exit 1` |
| G3. style 단일행 검증 | `python -c "import re,sys; [sys.exit(1) for f in __import__('glob').glob('widgets/post*_widget.html') if re.search(r'<style[^>]*>[^<]*\n', open(f).read())]" && echo "FAIL: style 멀티라인"` |

**2단계 이후**: sound gain 수치·BH_SoundMuted·이모지 grep 순차 추가.
**G2·G4·G5**: 기존대로 납품 전 수동 확인 유지.
...
| 출처 | 약속 내용 |
|---|---|
| sound | `BH_queueSound(eventKey)` 단일 진입점으로만 효과음 호출. `BH_play*()` 직접 호출 금지 |
| design | 납품 보고서에 "body 미디어ID: XXX / og 미디어ID: YYY" 형식 명시 수신 |
| game_planner | 기획서 [BREAKS] Footer 표기 준수. 승인 태그(`plan/p{N}-v{X.Y}`) 기준 파일을 구현 베이스로 사용 — 파일시스템 최신본 기준 금지 |

**발신 인터페이스 (game이 주는 것)**:

| 수신처 | 약속 내용 |
|---|---|
| design_ui | BH_ 클래스명 변경 시 사전 통보 필수 (design_ui CSS 컴포넌트 의존성 있음) |
| sound | 기획서 [효과음 매핑] 섹션의 이벤트 key 명칭 그대로 `BH_queueSound()` 호출 |

### Q5. 태그 명명 규칙 — 영역별 prefix 표준 (A안 채택)

```
game 태그 형식: game/p{N}-{단계}-v{X}
예시: game/p7-fsm-v1, game/p7-wp-v1, game/p7-minify-v1
```

| 태그 시점 | 형식 예시 |
|---|---|
| FSM/로직 완성 | `game/p7-fsm-v1` |
| minify 완료 | `game/p7-minify-v1` |
| WP PUT 성공 | `game/p7-wp-v1` |

`git tag -l "game/*"` 으로 game 태그만 필터링 가능.

---

## 🔵 git 운영 룰 — game 에이전트 위젯 파일 관리 (★ 2026-05-25 신설 — Pro Git §3 학습)
...
| `fix` | 버그 수정 |
| `feat` | 신규 기능 추가 |
| `style` | CSS/UI 변경 (로직 무변경) |
| `refactor` | 내부 리팩터 (동작 동일) |
| `build` | minify 적용 |
| `deploy` | WP 업로드 완료 기록 |
...
# 주의: git reset --hard 사용 금지 — 작업 트리 전체 파괴 위험
```
