...
**버전**: v6.56 — 라이브 PATCH 전 로컬 검증 PASS 확인 필수 룰 추가 (2026-05-25)
...
## 🔴 라이브 PATCH 전 로컬 검증 PASS 확인 필수 (★ v6.56 신설 — 2026-05-25)

**사고 배경**: 2026-05-25 Post #7 FSM 통합 라이브 사고. game 에이전트 로컬 검증 없이 PATCH 진행 → 광 카드 잔존 버그 라이브 배포 → PATCH 4건 연쇄 대응 → 진짜 원인 미해결 → 사용자 GUI 복원 필요.

| 규칙 | 내용 |
|---|---|
| **PATCH 전 PASS 확인** | WP 게임 위젯 PATCH 실행 직전 lead·game 로컬 검증 PASS 결과 첨부 확인 필수 |
| **PASS 미확인 시 거부** | 로컬 PASS 결과 없으면 PATCH 실행 거부 — 이유 불문 |
| **거부 보고** | 거부 시 lead에 "로컬 검증 PASS 미첨부 — PATCH 거부" 사유 보고 |
| **dev 임의 진행 금지** | "급하다"·"간단한 수정이다" 이유로 dev 단독 판단 진행 절대 금지 |
| **연쇄 PATCH 금지** | 라이브 PATCH 후 버그 발생 시 추가 PATCH 반복 금지 — ABORT + 로컬 원인 특정 후 단일 재시도 |

> ⚠️ PASS 첨부 없이 "빠르게 수정하자"는 요청은 이 룰 위반 — 거부 후 lead에 보고
