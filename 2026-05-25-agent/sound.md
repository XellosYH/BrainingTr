...
**버전**: v1.6 — git 운영 룰 + BH_queueSound 단일 진입점 (2026-05-25)
...
### Q1. 위젯 파일 구조 — 2단계 분리 정책 (B안 채택 — sound 도메인 한정)

| 파일 | 역할 | git 추적 |
|---|---|---|
| `widgets/post{N}_sound_module.js` | 멀티라인 주석 포함 원본 (파라미터 diff 추적용) | ✅ 추적 (원본) |
| `widgets/post{N}_widget.html` | minify 통합 납품 정본 (sound 인라인 포함) | ✅ 추적 (정본) |

**원칙**: `post{N}_sound_module.js` 멀티라인 원본이 git 추적 대상. gain 수치·주파수 파라미터 변경이 diff에 명확히 보임. minify 결과는 game 위젯 통합 시점 커밋에 포함.

**CLAUDE.md §위젯 파일 단일 정본 룰 준수**: `post{N}_widget.html` 납품 정본은 단일 파일. `sound_module.js`는 개발 원본으로 별도 유지 허용.

### Q2. LFS 도입 임계값 — 50포스트/200MB 초과 시

sound 산출물(`.js` 텍스트)은 LFS 대상 아님. `git lfs track` 패턴에 `*.js` 미포함 확인 필수. design PNG 바이너리 누적이 임계값 초과 시 dev 에이전트가 LFS 도입 — sound 작업 절차 변경 없음.

### Q3. pre-commit hook — 단계적 도입 (B안 채택)

**sound 담당 4개 정적 분석 게이트** (dev 에이전트가 구현 통합):

```bash
# 1. gain 수치 초과 검증 (0.3 이하 필수)
grep -rn "gain\.setValueAtTime(0\.[4-9]\|gain\.setValueAtTime([1-9]" widgets/post*_sound_module.js && echo "FAIL: gain > 0.3 초과" && exit 1

# 2. BH_SoundMuted 체크 누락 검증
grep -rn "function BH_play" widgets/post*_sound_module.js | grep -v "BH_SoundMuted" && echo "FAIL: BH_SoundMuted 체크 누락" && exit 1

# 3. BH_getAC() 미경유 직접 AudioContext 생성 금지
grep -rn "new AudioContext\|new webkitAudioContext" widgets/post*_sound_module.js && echo "FAIL: BH_getAC() 미경유 직접 호출" && exit 1
```

**try-catch 래핑 미적용 검증**: sound 에이전트 초안 작성 → dev가 `.git/hooks/pre-commit` 통합.

### Q4. 인터페이스 약속 — 즉시 일괄 반영 (A안 채택)

**발신 인터페이스 (sound가 주는 것)**:

| 수신처 | 약속 내용 |
|---|---|
| game | `BH_queueSound(eventKey)` 단일 진입점. game은 `BH_play*()` 직접 호출 금지 |
| game | BGM GainNode를 `window.BH_BGM_GainNode`에 전역 저장 — dev·game이 ducking 등 접근 가능 |

**수신 인터페이스 (sound가 받는 것)**:

| 출처 | 약속 내용 |
|---|---|
| game_planner | 기획서 [효과음 매핑] 섹션의 이벤트 key 명칭 — sound가 그대로 `BH_queueSound()` 매핑 |
| game | `BH_showFeedback()` 타이밍에 맞춰 효과음 즉시 재생 (design_ui 피드백 UI와 동기) |

**BH_queueSound 단일 진입점 표준**:

```js
// ✅ game 에이전트가 sound 호출 시 반드시 이 방식만 사용
BH_queueSound('card_place');   // 카드 놓기
BH_queueSound('match_win');    // 매칭 성공
BH_queueSound('game_over');    // 게임 종료

// ❌ 직접 호출 금지
BH_playCardPlace();  // 금지 — BH_queueSound 경유 필수
```

**뮤트 상태 localStorage 저장 표준** (재방문 시 유지):

```js
window.BH_SoundMuted = localStorage.getItem('BH_SoundMuted') === 'true';
```

### Q5. 태그 명명 규칙 — 영역별 prefix 표준 (A안 채택)

```
sound 태그 형식: sound/p{N}-v{X.Y}
예시: sound/p7-v1.5, sound/p7-v1.6
```

기존 `sound-v1.5-post7` 형식을 `sound/p{N}-v{X.Y}` 로 변경. `git tag -l "sound/*"` 으로 sound 태그만 필터링 가능.

---

## 🔊 git 운영 규칙 — sound 모듈 버전 관리 (★ 2026-05-25 신설 — Pro Git §3 학습)

**운영 환경**: Windows (Claude Code / PowerShell) 전용. Cowork Linux 샌드박스에서 git 실행 금지.

### 커밋 단위

효과음 함수 1종 추가·수정 = 커밋 1개. 커밋 메시지에 파라미터 이전값→이후값 명시.

```
# 형식
sound: BH_{함수명} {변경 요약}

이전: gain=0.2, freq=440Hz
이후: gain=0.15, freq=523Hz (C5, 정답음 개선)
```
...
```bash
# 동일 이벤트 효과음 여러 버전 비교
git checkout -b tune-BH_playMatch-v1   # 버전 A 실험
git checkout -b tune-BH_playMatch-v2   # 버전 B 실험

# 채택 후 main에 merge
git merge --no-ff tune-BH_playMatch-v1

# 미채택 브랜치 삭제
git branch -D tune-BH_playMatch-v2
```

### WP 업로드 태그

```bash
git tag -a sound/p{N}-v{X.Y} -m "Post{N} sound 모듈 v{X.Y} WP 업로드"
git push origin sound/p{N}-v{X.Y}
```
