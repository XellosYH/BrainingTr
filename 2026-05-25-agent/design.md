...
**버전**: v6.39 — git 운영 룰 + LFS 모니터링 책임 (2026-05-25)
...
design 납품 파일명도 suffix 없이 2버전 표준 적용. 구조 일관성 유지.

| 규칙 | 내용 |
|---|---|
| **body 버전 파일명** | `p{N}c0{n}_body.png` — suffix 없이 단일 파일명 고정 |
| **og 버전 파일명** | `p{N}c0{n}_og.png` — suffix 없이 단일 파일명 고정 |
| **버전 히스토리** | git 커밋으로 관리. 파일명 버전 suffix 금지 |
| **개발용 분리본** | `_draft.svg`, `_working.svg` 등 개발 임시 파일은 납품 후 삭제 또는 `.gitignore` 처리 |

### Q2. LFS 도입 임계값 — 50포스트/200MB 초과 시 (design 핵심 도메인 의견)

design이 PNG 바이너리 자산 주생산자이므로 LFS 도입 시점 판단 책임 보유.

| 지표 | 임계값 | 내용 |
|---|---|---|
| 포스트 수 | 50개 초과 | 포스트당 PNG 평균 6장 × 300KB = 약 1.8MB |
| 저장소 용량 | 200MB 초과 | 히어로 이미지·썸네일 포함 시 도달 |
| LFS 도입 시 | dev 에이전트에 보고 | `git lfs track "design/*.png"` 도입 요청 |

**커밋 메시지에 파일 크기 명시** (LFS 도입 전 용량 추이 추적):

```
asset: p7c01_body.png 287KB — design_ui 흰배경 전환
asset: p7c01_og.png 312KB — 소셜 OG 컬러 배경
```

### Q3. pre-commit hook — 단계적 도입 (B안 채택)

**design 담당 hook 항목** (dev 에이전트가 구현 통합):

```bash
# SVG NUL 패딩 검출
file design/hwatu_svg/*.svg | grep -c NUL && echo "FAIL: SVG NUL 패딩 검출" && exit 1

# 라이선스 마커 누락 grep (CC0·퍼블릭도메인 표시 의무 자산)
grep -rL "<!-- license:" design/hwatu_svg/ && echo "WARN: 라이선스 마커 누락 SVG 있음"
```

### Q4. 인터페이스 약속 — 즉시 일괄 반영 (A안 채택)

**발신 인터페이스 (design이 주는 것)**:

| 수신처 | 약속 내용 |
|---|---|
| design_ui | SVG 납품 시 `viewBox`만 설정 + `width="100%" height="100%"` 필수 (design_ui 컴포넌트 크기 의존) |
| dev | 납품 보고서에 "body 미디어ID: XXX / og 미디어ID: YYY" 형식 명시 (dev WP REST API 연결용) |

**수신 인터페이스 (design이 받는 것)**:

| 출처 | 약속 내용 |
|---|---|
| game_planner | 기획서 [UI 요구사항] 섹션에 카드 종류(광/조/피) 구분 명시 |

### Q5. 태그 명명 규칙 — 영역별 prefix 표준 (A안 채택)

```
design 태그 형식: design/p{N}-{자산}-v{X}
예시:
  design/p7-gwang-v1      (광 카드 완성)
  design/p7-all-v1        (전체 카드 완성)
  design/hwatu-gwang-v1   (화투 광 그룹 완성)
```

`git tag -l "design/*"` 으로 design 태그만 필터링 가능.

---

## 🖼️ git 버전 관리 운영 룰 (design 자산 전용) (★ 2026-05-25 신설 — Pro Git §3 학습)

**운영 환경**: Windows (Claude Code / PowerShell) 전용. Cowork Linux 샌드박스에서 git 실행 금지.

### 바이너리 자산 커밋 원칙

PNG 바이너리는 git diff가 의미 없음 — 커밋 메시지로 변경 내용을 명확히 기술.
...
```bash
# 카드 그룹 완성 시 태그 생성
git tag -a design/p{N}-gwang-v1 -m "Post{N} 광 카드 5장 완성"
git push origin design/p{N}-gwang-v1

# 전체 카드셋 완성 시
git tag -a design/p{N}-all-v1 -m "Post{N} 48장 전체 완성"
```
...
저장소 200MB 초과 또는 `git push` 현저히 저하 시 → dev 에이전트에 LFS 도입 보고.

### 자가점검 체크리스트 (납품 완료 시)

- body + og 2버전 각각 커밋 ✅
- 커밋 메시지에 파일 크기 명시 ✅
- 납품 보고서에 body/og 미디어ID 각각 명시 ✅
- design/p{N}-v{X} 태그 생성 + push ✅
